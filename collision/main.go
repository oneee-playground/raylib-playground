...
var (
	...
	wall = rl.NewRectangle(500, 250, 18, 60)
)
...
func load() {
	...
	player.cam = rl.NewCamera2D(rl.NewVector2(windowWidth/2, windowHeight/2), rl.NewVector2(
...
func getInputAndAct() {
	...
	case rl.IsKeyDown(rl.KeySpace):
		...
		posX := player.positionRect.X + (player.positionRect.Width / 2)
	...
	if player.deltaX != 0 {
		...
		if rl.CheckCollisionRecs(player.positionRect, wall) {
			xPos := wall.X - player.positionRect.Width
			if player.deltaX < 0 {
				xPos += wall.Width + player.positionRect.Width
			}
			player.positionRect.X = xPos
		}
	}
	...
	if !player.isOnFloor {
		...
		if rl.CheckCollisionRecs(player.positionRect, wall) {
			player.positionRect.Y = wall.Y + wall.Height
			player.deltaY = 0
		}
	...
	for _, p := range player.projectiles {
		if p.isActive {
			...
			if diff > float64(windowWidth/2) || rl.CheckCollisionRecs(p.positionRect, wall) {
				p.isActive = false
			}
...
func drawScene() {
	...
	rl.DrawRectangleRec(wall, rl.Brown)

	rl.DrawRectangleRec(player.positionRect, rl.Gray)
