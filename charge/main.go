...
func getInputAndAct() {
	...
	case rl.IsKeyDown(rl.KeySpace):
		...
		if !player.isCharging {
			...
			player.curProjectile = &projectile{
	...
	if player.isCharging {
		if !isSpaceDown {
			...
			player.latestShot = time.Now()

			player.projectiles[player.projectileIdx] = player.curProjectile
			player.projectileIdx = (player.projectileIdx + 1) & len(player.projectiles)

			player.curProjectile = nil
		} else {
			player.curProjectile.positionRect.Width++
			player.curProjectile.positionRect.Height++
		}
	}
...
func drawScene() {
	...
	if player.curProjectile != nil {
		rl.DrawRectangleRec(player.curProjectile.positionRect, rl.Orange)
	}
